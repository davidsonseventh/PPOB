...
$search_cat = $call->query("SELECT * FROM category WHERE name = '$get_s' AND type IN ('aktivasi-voucher)");
...
<? unset($_SESSION['success']); else: ?>
    ...
    <div id="appCapsule" class="rgs-order">
                            ...
                            <div class="in">
                                <div class="form-group basic">
                                    <div class="input-wrapper">
                                        ...
                                        <input type="number" class="form-control" placeholder="Masukkan Nomoor Vocuher" name="data" id="aktivasi voucher" required>
                                        <i class="clear-input">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active" id="Server1" role="tabpanel">
                <div class="section rgs-list-layanan">
                    <?php
                    $search = $call->query("SELECT * FROM srv WHERE type IN ('aktivasi-voucher') AND brand = '$operator' AND provider = 'DIGI' ORDER BY price ASC");
            ...
            <div class="tab-pane fade" id="Server2" role="tabpanel">
                <div class="section rgs-list-layanan">
                    <?php
                    $search = $call->query("SELECT * FROM srv WHERE type IN ('aktivasi-voucher) AND brand = '$operator' AND provider = 'X' ORDER BY price ASC");
                    if($search->num_rows == FALSE) { ?>
                    <div class="alert alert-danger text-left fade show" role="alert">
                        Tidak Ada Layanan Yang Tersedia!
                    </div>
        ...
        <div class="section rgs-list-layanan">
            <?php
            $search = $call->query("SELECT * FROM srv WHERE type IN ('aktivasi-voucher') AND brand = '$operator' ORDER BY price ASC");
            if($search->num_rows == FALSE) { ?>
            <div class="alert alert-danger text-left fade show" role="alert">
                Tidak Ada Layanan Yang Tersedia!
            </div>
