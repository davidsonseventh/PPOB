...
if(isset($_POST['buyprepaid'])) {
    ...
    if(check_bcrypt($postPin, $data_user['pin']) == true) {
        ...
        if($call->query("INSERT INTO trx VALUES ('', '$WebID', '$TrxID', '$sess_username', '$pcode', '$service', '$target', 'Transaksi Sedang Diproses', '{$data_service['price']}', '$profit', 'processing', '0', '0', '0', 'WEB,$pip', 'prepaid', '$date $time', '$date $time', '$server')") == true) {
            ...
            if($trx_result == true) {
                ...
                $_SESSION['success'] = ['type' => true, 'price' => $price, 'trxid' => $WebID];
                 echo '<script>location.reload();</script>'; // Replace with your transaction page
                exit();
            } else {
