...
if(isset($_POST['loginWhatsApp'])) {
        ...
        if(check_bcrypt($postPin, $data_user['pin']) == true) {
            ...
            if(isset($_COOKIE['FCM_TOKEN'])) {
                ...
                if($call->query("SELECT * FROM users_token WHERE user = '".$data_user['username']."' AND token = '$userFcm'")->num_rows == TRUE) {
                    ...
                    $call->query("INSERT INTO users_token VALUES (NULL,'".$data_user['username']."','$userFcm','$datetime')");
                }
            }
            ...
            $call->query("INSERT INTO logs VALUES (NULL, '".$data_user['username']."', 'login', '$ip', '$datetime')");
            unset($_SESSION['pin']);
    ...
    if($result_csrf == false) {
       ...
       $_SESSION['result'] = ['type' => false,'message' => 'Kode OTP Yang Anda Masukkan Salah, Harap Periksa Kembali.']; 
        ...
        $in = $call->query("INSERT INTO users VALUES (NULL,'$card','$post_name','".$data['email']."','$post_phone','".$data['user']."','0','0','0','Basic','$kodeReff','$reff','active','default.png','No','$datetime', '".bcrypt($data['pin'],10)."')");
        if($in == true){
