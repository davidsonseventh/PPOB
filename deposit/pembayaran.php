            ...
            if($berhasil == false) {
                ...
                if($ins == true) {
                    ...
                    elseif(in_array($post_method, ['alfamart', 'alfamidi','indomaret'])) : 
                        $textWa = 'Terima Kasih telah melakukan Deposit menggunakan '.ucfirst($post_method).'.
                        Silahkan Datang ke '.ucfirst($post_method).' :
...
4. Simpan struk bukti pembayaran Anda';
                        $textFC = 'Terima Kasih telah melakukan Deposit menggunakan '.ucfirst($post_method).' 
                        Silahkan Datang ke '.ucfirst($post_method);
                    else :
                        $textWa = 'Hallo '.$data_user['name'].', Mohon Segera Lakukan Pembayaran ke '.$post_note2.' ('.$post_note3.') sebesar Rp '.currency($post_total).'.

Jika Sudah Silahkan Klik Tombol Konfirmasi di Halaman Rincian Pembayaran maka Saldo akan Masuk Otomatis

*NB* : Jika tidak Ada Tombol Konfirmasi Silahkan Tunggu sampai Saldo Masuk.';
